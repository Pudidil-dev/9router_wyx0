...
export async function GET(request, { params }) {
  try {
    ...
    if (action === "device-code") {
      ...
      const noPkceDeviceProviders = ["github", "kiro", "kimi-coding", "kilocode", "codebuddy", "codebuddy-cn", "qoder"];
      let deviceData;
...
export async function POST(request, { params }) {
  try {
    ...
    if (action === "poll") {
      ...
      // Providers that don't use PKCE for device code
      const noPkceProviders = ["github", "kimi-coding", "kilocode", "codebuddy", "codebuddy-cn"];
      let result;
