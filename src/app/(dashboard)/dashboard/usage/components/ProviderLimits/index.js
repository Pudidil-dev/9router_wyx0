    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {renderedConnections.map((conn) => {
          ...
          const isCodex = !isAggregate && conn.provider === "codex";
          const resetCreditCount = isCodex ? getCodexResetCreditCount(quota) : 0;
          const isResettingLimit = !isAggregate && resettingLimitId === conn.id;
          const rowBusy = isAggregate ? false : deletingId === conn.id || togglingId === conn.id || isResettingLimit;
