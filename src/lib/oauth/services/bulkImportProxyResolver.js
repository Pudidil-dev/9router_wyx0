...
function buildResolvedProxy(proxyUrls, source) {
  ...
  return {
    ...
    proxyUrls: urls,
    proxyMode:
      urls.length > 1 ? "round-robin" : urls.length === 1 ? "single" : "none",
 ...
 *   3. settings.outboundProxyEnabled + settings.outboundProxyUrl fallback
 *
...
export async function resolveBulkImportProxy({ proxyPoolId, proxyUrl } = {}) {
  ...
  if (proxyUrl) {
    ...
    if (!proxyUrls.length)
      return buildResolvedProxy([], { proxySource: "custom" });
    const validationError = validateProxyUrls(proxyUrls);
    if (validationError) {
      return {
        proxyUrl: null,
