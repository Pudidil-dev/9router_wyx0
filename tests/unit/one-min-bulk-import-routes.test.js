...
vi.mock("@/lib/oauth/services/oneMinBulkImportManager", () => ({
  getOneMinBulkImportManager: vi.fn(() => managerMock),
  parseOneMinBulkAccounts: vi.fn((accounts = []) => {
    const parsed = [];
    const invalidLines = [];
...
describe("1min AI bulk import routes", () => {
  ...
  it("cancels a known job", async () => {
    ...
    const { POST } = await import("../../src/app/api/oauth/1min-ai/bulk-import/[jobId]/cancel/route.js");
